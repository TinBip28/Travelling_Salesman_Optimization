...
class PSO:
    ...
    def __init__(
        ...
        iterations,
        population_size,
        gbest_probability=1.0,
        pbest_probability=1.0,
        cities=None,
    ):
        self.cities = cities
        self.gbest = None
    ...
    def run(self):
        ...
        for t in range(self.iterations):
            ...
            if t % 20 == 0:
                plt.figure(0)
                plt.plot(pso.gcost_iter, "orange")
                plt.ylabel("Khoảng cách")
                plt.xlabel("Số lần lặp")
                fig = plt.figure(0)
                plt.legend(["Khoảng cách"])
                x_list, y_list = [], []
                ...
                fig = plt.figure(1)
                fig.clear()
                fig.suptitle(f"PSO TSP lần thứ {t}")
                plt.xlabel("x")
                plt.ylabel("y")
            ...
            for particle in self.particles:
                ...
                for i in range(len(self.cities)):
                    if new_route[i] != particle.pbest[i]:
                        swap = (
                            i,
                            particle.pbest.index(new_route[i]),
                            self.pbest_probability,
                        )
